...
fl1 = ggplot(dat, aes(x = flood_netsize)) + geom_bar(fill = "darkgrey", alpha = .8) + 
  labs(y = "Count",
       x = "External Network Size - Flooding") +
  theme_bw() + 
  theme(text = element_text(size = 12)) + 
  scale_y_continuous(limits = c(0, 135))
...
mod1.ev = glm(ev_netsize ~ tot_ev_units + dedicated.staffing, 
...
modelsummary(avg_slopes(mod3.ev, vcov = ~state),
             estimate = "{estimate} ({std.error}) {stars}",
             statistic = NULL,
             stars = T, fmt = 2,
             coef_rename = c(
...
slopes(mod4.ev, vcov = ~ state, variables = "dedicated.staffing", 
                by = "centralized.staffing")

slopes(mod3.ev, vcov = ~ state, variables = "tot_ev_units", 
                by = "centralized.staffing")
```
...
modelsummary(avg_slopes(mod2.fl, vcov = ~ state),
             estimate = "{estimate} ({std.error}) {stars}",
             statistic = NULL,
             stars = T, fmt = 2,
             coef_rename = c(
